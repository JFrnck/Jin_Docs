...
Zip-slip en `startPreviewService`: la clave de `files` se escribía sin sanear en el header USTAR (`tar-payload.ts`), y el init container la extrae con `tar xz -C /workspace` — una clave `"../../app/evil.js"` escapaba del directorio de trabajo. El `hitlLevel: confirm` de la tool no lo cubre (el owner aprueba un `planSummary`, no audita cada clave del mapa).

**Cerrado:** `isSafeRelativePath()` nueva (rechaza absolutos y cualquier segmento `..`, por segmento no substring — `"foo..bar.js"` sigue siendo válido) llamada tanto dentro de `buildTarGzBase64` (capa más profunda) como en `preview-service-request.schema.ts` vía `.refine()` (falla con 400 en el borde HTTP). Esta es la corrección primaria y queda confirmada por CI real.

**Intentado y revertido en el mismo PR, tras fallar CI real (no una sospecha):** se agregó `readOnlyRootFilesystem: true` al container `app` como defensa en profundidad adicional — antes de decidirlo se había revisado el único test de integración con K3s vivo (usa `require("http")` puro, sin escritura a disco) para descartar una regresión obvia. El CI del PR corrió igual el smoke test real y lo colgó: **4 minutos de silencio total en el log**, sin que ni siquiera el timeout propio de `waitUntilPodRunning` (60s, con su propio mensaje de error) llegara a dispararse — distinto de la flakiness de red ya documentada en ADR 0003 (esa es rápida, con reintentos que sí loguean). Sin acceso a un clúster K3s real para diagnosticar la causa exacta (Docker apagado en esta máquina), se revirtió en vez de forzar un cambio de runtime sin poder verificarlo. Queda como **punto 28 nuevo, abierto**, en `RECOMENDACIONES.md` — no bloquea nada, el zip-slip no depende de esto.

**Punto 26 (mismo PR, no afectado por el revert):** el init container `extract-workspace` no declaraba `resources`, dependía enteramente del `LimitRange` de `agents-sandbox` (Jin_Infra, otro repo). Ahora declara los suyos (250m/256Mi request, 500m/512Mi limit).
