...
## Última actualización: 2026-08-02 (America/Lima) — actualización 20
...
- **Repo:** [Jin_Web] — sin rama abierta. **Bloqueado a propósito, esperando el diseño visual del owner** (ver abajo). Fase 6.1 (Jin_Core) mergeada — ver sección dedicada abajo.
- **Descripción:** `Jin_Web` reasignado de Antigravity a Claude Code el 2026-08-02 (decisión del owner). Próxima tarea: Fase 6.2 (Web Dashboard MVP), `docs/PROMPTS.md` 6.2 ya re-etiquetado `[CLAUDE CODE]`.
- **Estado 2026-08-02:** escrito `docs/WEB_DESIGN_BRIEF.md` — prompt listo para pegar en claude.ai/design, con el inventario de funcionalidades sacado del **código real** (`registry.ts`: 16 tools con sus niveles HITL; `contracts/openapi.json`: 10 endpoints + eventos WS), no del blueprint. Cubre las 10 pantallas, los 4 niveles HITL como sistema visual central, y los requisitos PWA de §8.1.1.
- **Próximo:** el owner genera el diseño en Claude Design. **Decisión explícita de NO arrancar la implementación antes** — construir la UI ahora significaría rehacerla cuando llegue el diseño. Al volver: traer el diseño (vía `DesignSync` o pantallas pasadas a mano), y recién ahí abrir `feature/claude/dashboard-mvp` y proponer el plan de 6.2 sobre esa base visual.
